...
class _MessagesScreenState extends State<MessagesScreen> {
  ...
  Widget build(BuildContext context) {
    // if(widget.chatName != 'new'){
      context.read<ChatCubit>().emitGetChatMessagestates(widget.chatName);
    // }

    return Scaffold(
      backgroundColor: ColorsManager.lightDarkColor,
          ...
          child: Column(
            children: [
                        ...
                        child: Column(
                          children: [
                            Column(
                              ...
                              children: [
                                ChatWidget(
                                  msg: ChatCubit.chatList[index].message
                                      .content, // chatList[index].msg,
                                  chatIndex: (msg[index].index), //chatList[index].chatIndex,
                            ...
                            state is Loading && (index == msg.length - 1)
                                ? InterActionsWidget()
                                : const SizedBox(),
                          ],
                        ),
              ...
              Padding(
                ...
                child: Material(
                  color: ColorsManager.grayMedium, //cardColor,
                    ...
                    child: Row(
                      children: [
                        ...
                        GestureDetector(
                          onTap: () async {
                            if(widget.chatName == 'new'){
                              ChatCubit.get(context).emitAddChatStates(textEditingController.text);
                              setState(() async{
                                widget.chatName = textEditingController.text;
                                await sendMessageFCT();
                              });
                            }else {
                              await sendMessageFCT();
  ...
  Future<void> sendMessageFCT({String? nameChat}) async {
    ...
    try {
      ...
      context.read<ChatCubit>().addUserMessage(
          msg: ChatModel(message: Message(content: msg), index: 1),
          chatName: widget.chatName);
      textEditingController.clear();
      await context.read<ChatCubit>().emitSendMessageStates(UserMessage(
          message: [Message(content: msg)], model: 'gpt-3.5-turbo-0301'), widget.chatName);
      scrollListToEND();
      // setState(() {
      //
      // });
